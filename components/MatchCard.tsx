      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-2 md:gap-4 text-lg font-bold">
            <div
              className={`flex items-center gap-2 w-28 md:w-36 justify-end ${isTeamA ? "text-blue-300" : "text-white"}`}
            >
              <span>{getTeamName(match.teamA.id)}</span>
              <Flag
                code={match.teamA.id}
                className="w-8 h-5 object-cover border border-gray-700 rounded-sm shrink-0"
              />
            </div>
            ...
            <div
              className={`flex items-center gap-2 w-28 md:w-36 justify-start ${isTeamB ? "text-blue-300" : "text-white"}`}
            >
              <Flag
                code={match.teamB.id}
                className="w-8 h-5 object-cover border border-gray-700 rounded-sm shrink-0"
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="text-sm font-bold text-blue-400 w-16 text-center">{match.status}</div>
          ...
          {broadcasters.length > 0 ? (
            <div className="space-y-3">
              {broadcasters.map((b, index) => (
                <div
                  key={`${b.id}-${index}`}
                  className="bg-gray-900/50 rounded-lg p-4 border border-gray-700 flex flex-col md:flex-row md:items-center gap-4"
                >
                  {/* 放送局バッジ */}
                  <div className="w-24 shrink-0">
                    <span
                      className={`px-2 py-1 text-xs font-bold border rounded-md inline-block text-center w-full ${getBadgeStyle(b.type, b.id)}`}
                    >
                      {b.name}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-gray-300">
                    <p>
                      <span className="text-gray-500 mr-2 text-xs">実況:</span>
                      {b.playByPlay}
                    </p>
                    <p>
                      <span className="text-gray-500 mr-2 text-xs">解説:</span>
                      {b.commentator}
                    </p>
                  </div>
                </div>
