        ...
        <div className="w-48 flex flex-col pt-6 pr-4 shrink-0 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {openSidebar === "schedule" && (
            <ul className="space-y-2">
              ...
              <li>
                <button onClick={() => { setActiveMenu1("schedule"); setActiveMenu2("all"); setIsGroupExpanded(false); setOpenSidebar(null); }} className={`w-full text-left p-2 rounded ${activeMenu2 === "all" ? "bg-gray-800 text-white font-bold" : "text-gray-400 hover:bg-gray-800"}`}>全体日程</button>
              </li>
              <li>
                <button onClick={() => { setActiveMenu1("schedule"); if(activeMenu2.startsWith("group")) setIsGroupExpanded(!isGroupExpanded); else { setActiveMenu2("group"); setIsGroupExpanded(true); } }} className={`w-full text-left p-2 rounded flex justify-between items-center ${activeMenu2.startsWith("group") ? "bg-gray-800 text-white font-bold" : "text-gray-400 hover:bg-gray-800"}`}>
                  グループ別
                </button>
                {isGroupExpanded && (
              ...
              <li>
                <button onClick={() => { setActiveMenu1("schedule"); setActiveMenu2("japan"); setIsGroupExpanded(false); setOpenSidebar(null); }} className={`w-full text-left p-2 rounded ${activeMenu2 === "japan" ? "bg-gray-800 text-white font-bold" : "text-gray-400 hover:bg-gray-800"}`}>日本代表</button>
              </li>
            </ul>
          )}
          {openSidebar === "team" && (
            <ul className="space-y-2">
              ...
              {teams.map((team) => (
                <li key={team.id}>
                  <button onClick={() => { setActiveMenu1("team"); setActiveMenu2(team.id); setOpenSidebar(null); }} className={`w-full text-left p-2 rounded flex items-center gap-2 ${activeMenu2 === team.id ? "bg-gray-800 text-white font-bold" : "text-gray-400 hover:bg-gray-800"}`}>
      ...
      <div className="flex-1 p-8 overflow-y-auto bg-gray-900 text-white min-w-0 rounded-2xl shadow-2xl ml-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-gray-800 pb-4 mb-6">
          <h1 className="text-2xl font-bold text-white mb-4 md:mb-0">
            ...
            {activeMenu1 === "schedule" && activeMenu2.startsWith("group-") && `グループ ${activeMenu2.split("-")[1]}`}
